...
// Adapted from React Bits (https://reactbits.dev) — JellyRadio.
import {
  forwardRef,
  useEffect,
  useLayoutEffect,
...
export type JellyItem = string | { value: string; label: ReactNode; icon?: ReactNode; disabled?: boolean };

type Chip3 = { x: MotionValue<number>; sx: MotionValue<number>; sy: MotionValue<number> };

const DEFAULT_ITEMS: JellyItem[] = ["Off", "Low", "Medium", "High", "Max"];
...
type ChipProps = { mv: Chip3; children: ReactNode } & Omit<ComponentPropsWithoutRef<typeof motion.button>, "style" | "children">;
...
export default function JellyRadio({
  ...
  useEffect(() => {
    ...
    applied.current = at;
    // Outside (controlled) changes also get the jelly motion — used when scrolling moves the dock.
    apply(at, false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [at]);
  useEffect(
