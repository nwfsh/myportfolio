...
// Adapted from React Bits (https://reactbits.dev) — RefineFrame.
...
export type RefineStatus = "queued" | "generating" | "refining" | "complete" | "error";
...
type Live = { status: RefineStatus; stageDuration: number; sweep: boolean; reduce: boolean };

const STAGES: Record<RefineStatus, Stage> = {
...
const TARGET: Partial<Record<RefineStatus, number>> = {
  queued: 0,
  generating: 0.5,
  refining: 0.875,
  complete: 1,
...
export default function RefineFrame({
  ...
  const live = useRef<Live>({ status, stageDuration, sweep, reduce: false });
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Reset so wake() can restart the loop after a remount (React Strict Mode does this in dev).
      s.raf = 0;
      s.last = 0;
    };
  }, []);
